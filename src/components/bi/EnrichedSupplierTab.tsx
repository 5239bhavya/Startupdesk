...
import {
    ...
    Loader2, RefreshCw, Sparkles, Info, Scale, Search, Plus,
} from "lucide-react";
...
function getDefaultChips(businessType: string): string[] {
    ...
    // Generic fallback — use business name itself as first chip
    return [businessType, "Office supplies", "Packaging material", "Equipment & tools", "Raw materials"];
}
...
export function EnrichedSupplierTab({ materials, businessType, city = "India", lang = "en" }: EnrichedSupplierTabProps) {
    // All chips: plan rawMaterials + AI-suggested defaults
    const planChips = materials.map(m => m.name);
    const defaultChips = getDefaultChips(businessType);
    // Merge: plan chips first, then default chips not already in plan
    const allChips = [...planChips, ...defaultChips.filter(c => !planChips.some(p => p.toLowerCase() === c.toLowerCase()))];
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                    {allChips.slice(0, 8).map(chip => (
                        <Button
                            key={chip}
                            variant={selectedChip === chip && hasLoaded ? "default" : "outline"}
                            size="sm"
                                    ...
                                    <div className="flex flex-col md:flex-row gap-4">
                                        ...
                                        <div className="flex-1 space-y-3">
                                            <div className="flex items-center gap-3 flex-wrap">
                                                ...
                                                <Badge className={s.export_capable
                                                    ? "bg-emerald-500/15 text-emerald-700 border-emerald-500/30 text-xs"
                                                    : "bg-muted text-muted-foreground text-xs"}>
                                                    {s.export_capable ? t.exportYes : t.exportNo}
                                                </Badge>
                                            </div>
